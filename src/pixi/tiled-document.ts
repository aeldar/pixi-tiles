...
import { documentSizes, tileImgUrlsForDocumentAndLod, type DocumentId, type Lod } from './mock-meta-data';
...
export class TiledDocument extends PIXI.Container implements OnHandleZoomedEnd {
  ...
  #debugParams(event: Viewport) {
    console.log('Current Container World Bounds:', this.getBounds(true));
    console.log('Bounds size in pixels', this.getBounds(true).width, this.getBounds().height);
  }
  ...
  #addTiles(lod: Lod, newWidth: number, newHeight: number): PIXI.Container {
    ...
    console.log('Original size before adding tiles:', originalWidth, originalHeight);
    ...
    // const loadedCount = 0;
    // const totalTiles = urls.reduce((sum, row) => sum + row.length, 0);
    ...
    urls.forEach((row, m) => {
      row.forEach((url, n) => {
        ...
        const spriteWidth = isLastCol ? width % TILE_SIZE_PX || TILE_SIZE_PX : TILE_SIZE_PX;
        const spriteHeight = isLastRow ? height % TILE_SIZE_PX || TILE_SIZE_PX : TILE_SIZE_PX;
        ...
        PIXI.Assets.load(url)
          .then((texture) => new Promise<typeof texture>((resolve) => setTimeout(() => resolve(texture), 300)))
          .then((texture) => {
            ...
            // loadedCount++;
            // // After all tiles are loaded, rescale the container
            // if (loadedCount === totalTiles) {
            //   // The container's bounds after all sprites are added
            //   const bounds = c.getLocalBounds();
            //   const scaleX = originalWidth / bounds.width;
            //   const scaleY = originalHeight / bounds.height;
            //   c.scale.set(scaleX, scaleY);
            // }
          });
      });
    });

    // Ensure the tiles container fits the parent's size
